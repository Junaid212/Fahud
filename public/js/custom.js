...
(function ($) {
	...
	function slider_home_two() {
		const swiper = new Swiper(".Fahud-sl-2", {
			//effect: "coverflow",
			grabCursor: true,
			slidesPerView: 2,
			slidesPerGroup: 1,
	...
	window.initProjectsFilterSlider = function() {
		if (jQuery('.pro-filtr-cate-bx').length) {
			if (jQuery('.pro-filtr-cate-bx').hasClass('swiper-initialized')) return; // Prevent double init
			const config = {
			...
			// Show the container once initialized to prevent FOUC (full screen images while loading)
			jQuery('.pro-filtr-cate-bx').css('opacity', '1');
		}
	}
   ...
   ===========================*/
	...
	jQuery(document).on('submit', 'form.cons-contact-form', function (e) {
		...
		/* sending message */
		jQuery.ajax({
			url: 'https://thewebmax.org/Fahud/phpmailer/mail.php',
